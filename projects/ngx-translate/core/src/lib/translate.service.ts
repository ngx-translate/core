...
import {BehaviorSubject, concat, merge, Observable, Observer, of, Subject} from 'rxjs'
...
declare interface ILoadingTranslation {
}
...
export class TranslateService {
  ...
  // private pendingEpics: { [epicName: string]: boolean } = {}
  private pendingEpics: string[] = []
  ...
  public load(epic: string): void {
    if (!this.loadingTranslations) {
      this.loadingTranslations = new Subject<any>()
    }
    const loadingTranslations = this.loadingTranslations as Subject<any>
    if (!this.pendingEpics[epic]) {
      this.pendingEpics.push(epic)
      const lang = this._currentLang
      const loading = this.currentLoader.getTranslation(`${epic}-${lang}`)
        .pipe(
          take(1),
          map((res: Object) => this.compiler.compileTranslations(res, lang)),
          share()
        )
      this.pendingEpics[epic] = loading
        loading.subscribe((translations: Object) => {
          this.translations[lang] = mergeDeep(this.translations[lang], translations)
          const index = this.pendingEpics.indexOf(epic)
            if (index >= 0) {
              this.pendingEpics.splice(index,1)
            }
            loadingTranslations.next(this.translations[lang])
          }, (err: any) => {
            this.pending[epic] = false
          }
        )
    }
  }
  ...
  public get(key: string | Array<string>, interpolateParams?: Object): Observable<string | any> {
    ...
    } else if (this.pendingEpics.length) {
      return Observable.create((observer: Observer<string>) => {
        let onComplete = (res: string) => {
          observer.next(res)
        }
        ...
        merge(Object.keys(this.pendingEpics).map(k=>this.pendingEpics[k]))
          .subscribe((res: any) => {
          res = this.getParsedResult(res, key, interpolateParams)
          if (typeof res.subscribe === 'function') {
            res.subscribe(onComplete, onError)
