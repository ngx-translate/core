...
export const ISOLATE_TRANSLATE_SERVICE = new InjectionToken<string>(
...
export class TranslateService {
  ...
  private retrieveTranslations(
    ...
    if (typeof this.translations[lang] === "undefined" || this.extend) {
      this._translationRequests[lang] =
        this._translationRequests[lang] ||
        this.loadAndCompileTranslations(lang);
  ...
  public setTranslation(
    ...
    const interpolatableTranslations = this.compiler.compileTranslations(
      translations,
      lang,
    );
    if ((shouldMerge || this.extend) && this.translations[lang]) {
      this.translations[lang] = mergeDeep(
        this.translations[lang],
  ...
  private getParsedResultForKey(
    ...
    if (
      ...
      res = this.runInterpolation(
        getValue(this.translations[this.defaultLang], key),
        interpolateParams,
      );
    }
  ...
  private runInterpolation(
    ...
    if (isArray(translations)) {
      return (translations as Translation[]).map((translation) =>
  ...
  public getParsedResult(
    ...
    if (key instanceof Array) {
      ...
      for (const k of key) {
        result[k] = this.getParsedResultForKey(
          translations,
          k,
          interpolateParams,
        );
      ...
      const sources: Observable<Translation>[] = key.map((k) =>
        makeObservable(result[k]),
      );
      ...
      return this.loadingTranslations.pipe(
        concatMap((res: InterpolatableTranslation) => {
          return makeObservable(
            this.getParsedResult(res, key, interpolateParams),
          );
        }),
      );
    }
  ...
  public instant(
    ...
    const result = this.getParsedResult(
      this.translations[this.currentLang],
      key,
      interpolateParams,
    );
