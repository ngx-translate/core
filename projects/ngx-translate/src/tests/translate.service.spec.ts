...
describe("TranslateService", () => {
  ...
  const fakeNavigator: FakeNavigator =
    window.navigator as unknown as FakeNavigator;
  ...
  it("should be able to get translations with nested params", () => {
    ...
    translate
      .get("TEST", { param: { value: "with param" } })
      .subscribe((res: Translation) => {
        expect(res).toEqual("This is a test with param");
      });
  });
  ...
  it("should initially return the same value for streaming and non-streaming get", (done: DoneFn) => {
    ...
    zip(translate.stream("TEST"), translate.get("TEST")).subscribe(
      (value: Translation[]) => {
  ...
  describe("instant()", () => {
    ...
    it("should be able to interpolate in sub-trees", () => {
      ...
      expect(translate.instant("a.b", { value: "world" })).toEqual({
        x: "world 1",
        y: "world 2",
      });
    });
  ...
  describe("set()", () => {
    ...
    it("should translate 'default'", () => {
      ...
      expect(translate.instant("default")).toEqual(
...
describe("TranslateService (isolate)", () => {
  ...
  it("switches root and child component independently", async () => {
    ...
    expect(app.querySelector("div.isolated-child").textContent).toEqual(
      "de-child",
    );
    expect(app.querySelector("div.shared-child").textContent).toEqual(
      "en-root",
    );
