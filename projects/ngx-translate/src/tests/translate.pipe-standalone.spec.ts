...
class DelayedFrenchLoader implements TranslateLoader {
  getTranslation(lang: string): Observable<TranslationObject> {
    return lang === "fr"
      ? timer(10).pipe(map(() => translations))
      : of(translations);
  }
}
...
describe("TranslatePipe (standalone)", () => {
  ...
  const prepare = ({
    ...
    handlerClass?: Type<MissingTranslationHandler>;
    loaderClass?: Type<TranslateLoader>;
  } = {}) => {
      ...
      providers: [
        provideTranslateService({
          loader: {
            ...
            useClass: loaderClass ?? FakeLoader,
          },
          ...missingTranslationHandler,
  ...
  it("should translate 'default'", () => {
    ...
    expect(translatePipe.transform("default")).toEqual(
  ...
  it("should translate a string with object as multiple string parameters", () => {
    ...
    translate.setTranslation("en", {
      TEST: "This is a test {{param1}} {{param2}}",
    });
    translate.use("en");
  ...
  it("should translate a string with object as nested string parameters", () => {
    prepare();
    translate.setTranslation("en", {
      TEST: "This is a test {{param.one}} {{param.two}}",
    });
    translate.use("en");
  ...
  it("should translate an object", () => {
    ...
    expect(translatePipe.transform("OBJECT")).toEqual({
      TEST: "This is a test",
    });
  });
  ...
  it("should translate an object with another object as string parameters", () => {
    ...
    expect(
      translatePipe.transform("OBJECT", { param: "with param-1" }),
    ).toEqual({ TEST: "This is a test with param-1" });
  });
  ...
  describe("should update translations on default lang change", () => {
    it("with fake loader", (done) => {
      ...
      // this will be resolved at the next lang change
      const subscription = translate.onDefaultLangChange.subscribe(
        (res: DefaultLangChangeEvent) => {
          expect(res.lang).toEqual("fr");
          expect(translatePipe.transform("TEST")).toEqual("C'est un test");
          subscription.unsubscribe();
          done();
